...
const Calculator = {
    ...
    //This will hols the operator, we set it to null for now.
    operator: null,
};
...
function Handle_Operator(Next_Operator) {
    ...
    //displayed on the dcreen to a number and then store the result in
    //Calculator.First_Operand if it doesn't already exist.
    const Value_of_Input = parseFloat(Display_Value);
    //Checks if an operator already exists and if Wait_Second_Operand id true,
...
function Update_Display () {
    ...
    //input tag in thr HTML document
...
keys.addEventListener('click', (event) => {
    ...
    //Ensuresthat AC clears all inputs from the Calculator screen.
